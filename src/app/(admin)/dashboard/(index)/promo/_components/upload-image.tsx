...
export default function UploadImage({ data = "" }: UploadImageProps) {
  ...
  useEffect(() => {
    if (data) {
      setImage(getImageUrl(data, "promo"));
    }
  }, [data]);
  ...
  return (
    <div className="flex flex-col space-y-1.5">
      ...
      <div className="grid gap-2 border rounded-md p-2">
        <Image alt="Gambar Promo" onClick={openFolder} className="aspect-[7/3] w-full rounded-md object-cover border" height={900} width={2100} src={image || "/placeholder.svg"} />
        ...
        <button type="button" onClick={openFolder} className="flex gap-1 w-full p-4 items-center justify-center rounded-md border border-dashed">
          ...
          <span className="text-xs">Pilih Gambar Ukuran(3x7)</span>
        </button>
